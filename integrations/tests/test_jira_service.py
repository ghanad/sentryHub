...
class JiraServiceTests(TestCase):
    ...
    @patch('integrations.services.jira_service.JiraService.__init__', return_value=None)
    def test_add_watcher_client_none(self, mock_init):
        """Test add_watcher when client is None."""
        from integrations.services.jira_service import JiraService
        service = JiraService()
        service.client = None # Ensure client is None
        ...
        self.assertFalse(service.add_watcher(issue_key, username))

    @patch('integrations.services.jira_service.JiraService.__init__', return_value=None)
    @patch('integrations.services.jira_service.JIRA')
    def test_add_watcher_success(self, mock_jira_class, mock_init):
        """Test add_watcher with valid data."""
        from integrations.services.jira_service import JiraService
        service = JiraService()
        service.client = MagicMock() # Manually set client as __init__ is mocked
        ...
        self.assertTrue(service.add_watcher(issue_key, username))
        service.client.add_watcher.assert_called_once_with(issue_key, username)

    @patch('integrations.services.jira_service.JiraService.__init__', return_value=None)
    @patch('integrations.services.jira_service.JIRA')
    def test_add_watcher_empty_username(self, mock_jira_class, mock_init):
        """Test add_watcher with an empty username."""
        from integrations.services.jira_service import JiraService
        service = JiraService()
        service.client = MagicMock() # Manually set client as __init__ is mocked
        ...
        self.assertFalse(service.add_watcher(issue_key, username))
        service.client.add_watcher.assert_not_called() # add_watcher should not be called for empty username

    @patch('integrations.services.jira_service.JiraService.__init__', return_value=None)
    @patch('integrations.services.jira_service.JIRA')
    def test_add_watcher_jira_error(self, mock_jira_class, mock_init):
        """Test add_watcher when Jira API returns an error."""
        from integrations.services.jira_service import JiraService
        service = JiraService()
        service.client = MagicMock() # Manually set client as __init__ is mocked
        service.client.add_watcher.side_effect = JIRAError(status_code=404, text="Issue Not Found")
    ...
    @patch('integrations.services.jira_service.JiraService.__init__', return_value=None)
    @patch('integrations.services.jira_service.JIRA')
    def test_add_watcher_connection_error(self, mock_jira_class, mock_init):
        """Test add_watcher when a connection error occurs."""
        from integrations.services.jira_service import JiraService
        service = JiraService()
        service.client = MagicMock() # Manually set client as __init__ is mocked
        service.client.add_watcher.side_effect = ConnectionError("Mock connection error")
